...
import (
	...
	"encoding/json"
	"fmt"
	"server-monitoring/domain/language"
...
var (
	...
	childrenAdminTempllate []string
	childrenFrontTemplate  []string
	rootTemplate           string
	rootAdminTemp          string
	rootFrontTemp          string
...
type Template struct {
	...
	RootAdmin     string   `json:"RootAdmin"`
	RootFront     string   `json:"RootFront"`
	Children      []string `json:"Children"`
	ChildrenAdmin []string `json:"ChildrenAdmin"`
	ChildrenFront []string `json:"ChildrenFront"`
}
...
func LoadTemplates(rootTemp string, rootAdminx string, rootFrontx string, childTemps []string, childAdminTemp []string, childFrontTemp []string) {
	rootTemplate = rootTemp
	rootAdminTemp = rootAdminx
	rootFrontTemp = rootFrontx
	childTemplates = childTemps
	childrenAdminTempllate = childAdminTemp
	childrenFrontTemplate = childFrontTemp
}
...
func New(req *http.Request) *View {
	...
	// Set the AuthLevel to auth if the users is logged in
...
func (v *View) RenderFront(w http.ResponseWriter) {
	...
	if !ok || !viewInfo.Caching {
		...
		templateList = append(templateList, rootFrontTemp)
		templateList = append(templateList, v.Name)
		templateList = append(templateList, childrenFrontTemplate...)
	...
	// Display the content to the screen
	err := tc.Funcs(pc).ExecuteTemplate(w, rootFrontTemp+"."+v.Extension, v.Vars)
	//
	if err != nil {
		http.Error(w, "Template File Error: "+err.Error(), http.StatusInternalServerError)
	}
...
func (v *View) RenderAdmin(w http.ResponseWriter) {
	...
	if !ok || !viewInfo.Caching {
		...
		// List of template names
		var templateList []string
		templateList = append(templateList, rootAdminTemp)
		templateList = append(templateList, v.Name)
		templateList = append(templateList, childrenAdminTempllate...)
	...
	if sess.Values[consts.USER_ID] != nil {
		v.Vars["username"] = sess.Values[consts.USER_MAME]
		v.Vars["email"] = sess.Values[consts.USER_EMAIL]
		v.Vars["fullname"] = sess.Values[consts.FUll_NAME]
	}

	if sess.Values[consts.DIRECTION] != nil {
		v.Vars["direction"] = sess.Values[consts.DIRECTION].(bool)
	} else {
		v.Vars["direction"] = true
	}
	//if languages == nil {
	//	languages, _ = adminservice.SettingService.FindAllLanguage()
	//	v.Vars["langs"] = languages
	//} else {
	//	v.Vars["langs"] = languages
	//}

	// Display the content to the screen
	err := tc.Funcs(pc).ExecuteTemplate(w, rootAdminTemp+"."+v.Extension, v.Vars)
